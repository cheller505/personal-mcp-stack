...
def build_mcp_server(get_key: Callable[[], str]) -> Server:
    server = Server("granola-mcp")
    ...
    async def _call_tool(name: str, arguments: dict):
        return await handle_tool(name, arguments, get_key)

    return server


class _GranolaMCPApp:
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        if path == "/sse":
            ...
            body = b"Granola MCP server. Connect via /sse"
            await send({
...
async def run_server(
    ...
    mcp_server = build_mcp_server(get_key)
    sse = SseServerTransport("/messages")
    app = _GranolaMCPApp(sse, mcp_server)
